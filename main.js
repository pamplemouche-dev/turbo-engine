...
function selectObject(obj) {
    ...
    // On attache l'événement au sélecteur de couleur
...
function onPointerDown(event) {
    ...
    if (intersects.length > 0) {
        ...
        if (object !== baseplate && object !== grid && object.type === "Mesh") {
            selectObject(object);
        } else if (object === baseplate) {
            deselectObject();
        }
    }
}
...
window.addPart = function() {
    ...
    part.position.set(Math.random() * 20 - 10, 2, Math.random() * 20 - 10);
    scene.add(part);
